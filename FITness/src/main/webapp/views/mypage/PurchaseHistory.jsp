	...
	<div id="board-list-container">
		<table id="tbl-board" class="table table-hover table-striped text-center">
			...
			<c:if test="${ not empty list }">
				<c:forEach var="purchasehistory" items="${ list }">
					<tr>
						<td>${ purchasehistory.cate }</td> 
						<td>${ purchasehistory.gymName }</td>
						<td>${ purchasehistory.TPrice }</td>
						<td>${ purchasehistory.payMethod }</td>
						<td>${ purchasehistory.PDate }</td>
					</tr>
				</c:forEach>
			</c:if>
		</table>
		...
		<div id="pageBar">
			<nav aria-label="...">
  				<ul class="pagination justify-content-center">
    				<li class="page-item">
      					<a class="page-link" href='${ path }/board/myWriting?page=${ pageInfo.prevPage }'>previous</a>
   					 </li>
					...
					<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" varStatus="status">
						<c:choose>
							...
							<c:otherwise>
								<li class="page-item">
									<a class="page-link" href='${ path }/board/myWriting?page=${ status.current }'>${ status.current }</a>
								</li>
							</c:otherwise>
						</c:choose>
					</c:forEach>
    				...
    				<li class="page-item">
      					<a class="page-link" href='${ path }/board/myWriting?page=${ pageInfo.nextPage }'>next</a>
				    </li>
				 </ul>
			</nav>
		</div>

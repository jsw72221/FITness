...
<body>
    <div class="wrap">
            ...
            <div id="content4">
                <div id="content4_1">
                    <div id="board_navigator">
                        <ul id="board_navi">
                            <li><a href="${ path }/admin/boardnotice">NOTICE</a></li>
                            <li>
                ...
                <div id="content4_2">
                    <h3>자유게시판 &nbsp;&nbsp; <span>게시글 수 : &nbsp; ${ listCount }</span></h3>
                </div>
            </div>
            <div id="content2">
            <table>
             ...
             <c:forEach var="board" items="${ list }">
                <tr>
                    ...
                    <td>${ board.readCount }</td>
                     ...
                     <c:if test="${ board.status.equals('Y')}">
                     <td></td>
                     </c:if>
                     <c:if test="${ board.status.equals('N')}">
                     <td><button id="update" onclick="reportMemberStatus(${board.no}, '${ board.status }');">활성화</button></td>
                     </c:if>
                     <c:if test="${ board.status.equals('Y')}">
                    <td></td>
                     </c:if>
        ...
        <div id="content3">
        <!-- 맨 처음으로 -->
			<button onclick="location.href='${ path }/amdin/boardfree?page=1'">&lt;&lt;</button>

			<!-- 이전 페이지로 -->
			<button onclick="location.href='${ path }/admin/boardfree?page=${ pageInfo.prevPage }'">&lt;</button>
			...
			<c:forEach begin="${ pageInfo.startPage }" end="${ pageInfo.endPage }" varStatus="status">
				<c:choose>
					...
					<c:otherwise>
						<button onclick="location.href='${ path }/admin/boardfree?page=${ status.current }'">${ status.current }</button>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			...
			<!-- 다음 페이지로 -->
			<button onclick="location.href='${ path }/admin/boardfree?page=${ pageInfo.nextPage }'">&gt;</button>

			<!-- 맨 끝으로 -->
			<button onclick="location.href='${ path }/admin/boardfree?page=${ pageInfo.maxPage }'">&gt;&gt;</button>
        </div>
 ...
 <script>
    	function reportMemberStatus(no, status) {
    		
    			if(status === 'Y'){
    				if(confirm("부적절한 게시글로 처리하시겠습니까?")) {
    					location.href='${ path }/admin/boardreport?no=' + no + '&status=Y'
    				} else {
    					alert("아니오를 누르셨습니다.");
    				}
    			} else {
    				if(confirm("활성화 하시겠습니까?")) {
    					location.href='${ path }/admin/boardreport?no=' + no + '&status=N'
    				}
